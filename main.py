...
def _to_hex_tx_hash(h: Any) -> str:
    ...
    Accepts:
      ...
      - list/tuple of ints (32 bytes)
      - dicts containing hash fields
    ...
    if isinstance(h, dict):
        h = h.get("hash") or h.get("tx_hash") or h.get("txHash") or h.get("transactionHash") or ""
...
def _trade_lt_int(t: Dict[str, Any]) -> int:
    """Extract DeDust trade logical time (lt) if present (best-effort)."""
    if not isinstance(t, dict):
        return 0
    ...
    for k in ("lt", "tx_lt", "txLt", "logical_time", "logicalTime"):
        v = _as_int(t.get(k))
        if v:
            return v
...
def is_stonfi_dex(dex: str) -> bool:
    """True for STON.fi v1/v2 identifiers stored in data.json."""
    d = (dex or "").lower()
    return "ston" in d  # covers stonfi, ston.fi, stonfi-v2, stonfi2, etc.

def is_dedust_dex(dex: str) -> bool:
    d = (dex or "").lower()
    return "dedust" in d
...
def dedust_fetch_trades(pool_addr: str, limit: int = 25, after_lt: int = 0) -> List[Dict[str, Any]]:
    ...
    Uses api.dedust.io (public). Prefer cursor-based polling using after_lt
    to avoid reposting old trades. If the endpoint ignores after_lt, caller
    will still de-dup by lt/hash.
    """
    if not pool_addr:
        return []
    ...
    params: Dict[str, Any] = {}
    if limit:
        params["page_size"] = int(limit)
        params["limit"] = int(limit)
    if after_lt:
        params["after_lt"] = int(after_lt)
    try:
...
async def ston_tracker_job_fast(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for pool, rec in DATA.get("pairs", {}).items():
            if not isinstance(rec, dict):
                continue
            if not is_stonfi_dex(str(rec.get("dex", ""))):
                continue
        ...
        async def _fetch_pool(pool_addr: str):
            async with sem:
                txs = await _to_thread(tonapi_account_events, pool_addr, STON_TONAPI_LIMIT)
...
def extract_buy_from_ston_event(ev: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if not is_stonfi_dex(str(rec.get("dex", "stonfi"))):
        return None
...
def dedust_extract_buys_from_tonapi_tx(tx: Dict[str, Any], token_addr: str) -> List[Dict[str, Any]]:
    ...
    BUY = (native TON) -> token_addr.

    TonAPI action schemas change over time. This parser is intentionally tolerant:
      - merges nested payload (e.g. {type: 'DexSwap', DexSwap: {...}})
      - supports both flat fields (amount_in/amount_out, asset_in/asset_out)
        and nested (in/out objects).
      - treats the swap as a BUY if the output master matches token_addr and
        the input side is native TON.
    """
    out: List[Dict[str, Any]] = []
    tx_hash = _tx_hash(tx)
    ...
    for a in actions:
        ...
        key = a.get("type") or a.get("action") or a.get("name")
        payload = a.get(key) if isinstance(key, str) else None
        aa = dict(a)
        ...
        # Asset shapes (nested)
        sub_in = aa.get("in") or aa.get("from") or aa.get("input")
        if isinstance(sub_in, dict) and asset_in is None:
            asset_in = sub_in.get("asset") or sub_in.get("token") or sub_in.get("assetIn") or sub_in.get("inAsset")
        sub_out = aa.get("out") or aa.get("to") or aa.get("output")
        if isinstance(sub_out, dict) and asset_out is None:
            asset_out = sub_out.get("asset") or sub_out.get("token") or sub_out.get("assetOut") or sub_out.get("outAsset")

        # Determine out jetton master (tolerant)
        out_master = aa.get("jetton_master") or aa.get("jettonMaster") or aa.get("jetton")
        if isinstance(out_master, dict):
        ...
        # Determine in jetton master (helps skip sells)
        in_master = ""
        if isinstance(asset_in, dict):
            in_master = extract_jetton_master(asset_in)

        # Require output match (and skip obvious sells)
        if token_addr:
            if out_master and out_master != token_addr:
                continue
            if not out_master and isinstance(asset_out, dict):
                if extract_jetton_master(asset_out) and extract_jetton_master(asset_out) != token_addr:
                    continue
            if in_master and in_master == token_addr and (not out_master or out_master != token_addr):
                # token is on input side => likely a sell
                continue

        # Determine whether input side is TON
        input_is_ton = False
        if ton_in is not None:
            input_is_ton = True
        if is_ton_asset(asset_in):
            input_is_ton = True
        if aa.get("in_is_ton") is True or aa.get("inIsTon") is True:
            input_is_ton = True
        ...
        if amt_in is None and isinstance(sub_in, dict):
            amt_in = sub_in.get("amount") or sub_in.get("value") or sub_in.get("inAmount") or sub_in.get("amountIn")
        ...
        if amt_out is None and isinstance(sub_out, dict):
            amt_out = sub_out.get("amount") or sub_out.get("value") or sub_out.get("outAmount") or sub_out.get("amountOut")
        ...
        if ton_in is not None:
            if isinstance(ton_in, (int, float)):
                ...
                ton_spent = to_ton_from_nano(ton_in) if ton_in.isdigit() else safe_float(ton_in)
        elif amt_in is not None:
            # only treat amt_in as TON if we are confident input is TON
            if isinstance(amt_in, (int, float)):
                ...
                ton_spent = to_ton_from_nano(amt_in) if amt_in.isdigit() else safe_float(amt_in)

        # Parse token received
        token_received = 0.0
